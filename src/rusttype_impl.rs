...
impl FontProvider for Font<'_> {
    ...
    fn metrics(&self, key: GlyphKey) -> Metrics {
        ...
        let shape = glyph.positioned(Point {
            x: 0.0,
            y: 0.0,
        }).pixel_bounding_box().expect("The size of the glyph could not be calculated");
    ...
    fn rasterize(&self, key: GlyphKey) -> Vec<u8> {
        let glyph = scaled_glyph(self, key).positioned(Point {
            x: 0.0,
            y: 0.0,
        });
        let bounds = glyph.pixel_bounding_box().expect("The size of the glyph could not be calculated");
